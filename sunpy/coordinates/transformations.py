...
from astropy.coordinates import ICRS, HCRS, ConvertError, get_body_barycentric, SkyCoord
...
def hcc_to_hpc(helioccoord, heliopframe):
    ...
    if not _observers_are_equal(helioccoord.observer, heliopframe.observer):
        ...
        helioccoord = helioccoord.transform_to(heliocframe)

    # Shift the origin from the Sun to the observer
    distance = SkyCoord(helioccoord.observer).heliographic_stonyhurst.radius
    newrepr = helioccoord.cartesian - CartesianRepresentation(0*u.m, 0*u.m, distance)
